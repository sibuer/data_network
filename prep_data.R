...
library(httr)
library(jsonlite)
setwd("D:/OneDrive - Universitaet Bern/02_Vorlesungen/data_science/upload github")
# Read the CSV file
...
# only keep rows with abstract
tf_vec <- data$abstract==""|is.na(data$abstract)
data <- data[!tf_vec,]


## Set up Open AI Connection --------------------------------------------------

my_API <- "####-RvoEUTXjGctAgc01gaBRT3BlbkFJjAzBrKlGhAgWJMcSmpMI"
...
for(i in 1:dim(data)[1]){
    prompt_p1 <- 
      "summarize this abstract in 5 keywords (no keywords consisting of two words, 
    also no slashes or hyphen, only one word), 
    each separated by a comma (Make everything lower case). 
    The keywords can only be nouns, no adjectives.  
    make sure to use no unreletated keywords, only word related to the abstract presented\n"
